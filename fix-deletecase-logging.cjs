 ...
 * 2) Applies a safe patch replacing trial_binder_items.section_key -> trial_binder_items.section_key
 *    and common alias tbi.section_key -> tbi.section_key (only when trial_binder_items exists in the same file).
...
function patchTrialBinderSectionId(file, txt) {
  ...
  if (/trial_binder_items\.section_id\b/.test(updated)) {
    ...
    changes.push(`trial_binder_items.section_key → section_key (x${count})`);
  }
  ...
  if (/\btrial_binder_items\b/.test(updated) && /\btbi\.section_id\b/.test(updated)) {
    ...
    changes.push(`tbi.section_key → tbi.section_key (x${count})`);
  }
