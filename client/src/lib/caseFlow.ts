...
export function getNextModule(
  ...
  for (let i = currentIndex + 1; i < CASE_FLOW.length; i++) {
    ...
    if (!opts.hasChildren && (nextKey === "children" || nextKey === "child-support")) {
      continue;
    }
    return nextKey;
  }
...
export function getPrevModule(
  ...
  for (let i = currentIndex - 1; i >= 0; i--) {
    ...
    if (!opts.hasChildren && (prevKey === "children" || prevKey === "child-support")) {
      continue;
    }
    return prevKey;
  }
...
export function getVisibleModules(opts: { hasChildren: boolean }): ModuleKey[] {
  return CASE_FLOW.filter((key) => {
    if (!opts.hasChildren && (key === "children" || key === "child-support")) {
      return false;
    }
    return true;
  });
